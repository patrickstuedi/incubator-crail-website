...
<div style="text-align: justify"> 
<p>
In this blog we focus on Crail's KeyValue interface referred 
 </p>
</div>

```
CrailStore crail = CrailStore.newInstance();
CrailTable table = fs.create("/tab1", CrailNodeType.TABLE, CrailStorageClass.DEFAULT, CrailLocationClass.DEFAULT).get();
CrailKeyValue kv = fs.create("/tab1/key1", CrailNodeType.KEYVALUE, CrailStorageClass.DEFAULT, CrailLocationClass.DEFAULT).get();
