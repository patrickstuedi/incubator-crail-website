...
<div style="text-align: justify"> 
<p>
As can be seen, for small KV pairs (size 1K) Crail's update performance for the DRAM 
 
 
 is just a tiny bit off from RAMCloud, one of the fastest in-memory key-value stores out there. 
 
 Looking at the performance of Crail's Optane tier we notice that the latencies 
