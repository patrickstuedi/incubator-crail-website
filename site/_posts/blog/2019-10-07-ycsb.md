...
<p>
We used Crail's asynchronous API to benchmark Crail's key-value performance under load. The two figures below show the latency (shown on tye y-axis) of Crail's DRAM, Optane and Flash tiers under increasing load measured in terms of operations per second (shown on the x-axis).  
</p>
</div> 
