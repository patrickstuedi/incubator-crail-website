...
<p>
The figure below illustrates three possible configurations of Crail in a single rack: (left) non-disaggrageted where each each compute server exports some of its local DRAM and Flash into Crail by running one Crail storage server instance of both types. (middle) complete disaggregation where the compute servers do not participate in Crail storage but all 
 </p>
</div>
