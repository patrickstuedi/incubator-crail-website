...
Today, data processing applications running in the cloud may implicitly use disaggregated storage through cloud storage services like S3. For instance, it is not uncommon for mapreduce workloads in the cloud to use S3 instead of HDFS for storing the input and output data sets. While Crail can offer high-performance disaggregated storage for input/output data sets as well, in this blog we specifically look at how to use Crail for efficient disaggregation of shuffle data. 
