...
**Large number of small files:** The number and size of shuffle files depend on the workload, but also on the configuration, in particular on the number of map and reduce tasks in a stage of a job. The number of tasks in a job is often indirectly controlled through the partition size, the amount of data each task is operating on. Finding the optimal partition size for a job is difficult and often requires manual tuning. In theory, a small partition size and therefore a large number of small task helps to mitigate stragglers, a major issue for distributed data processing frameworks. In practice, configurations with very small partitions sizes often result in poor performance due to slow task scheduling and task startup times on the one hand side, but also due to excessive shuffle overheads.
